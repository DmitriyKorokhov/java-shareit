...
public class BookingServiceTest {
    ...
    void addBookingTest() {
        ...
        assertEquals(exception.getMessage(), "404 NOT_FOUND \"Некорректно задан пользователь\"");

        // not available
        item.setAvailable(false);
        exception = assertThrows(ValidationException.class, () -> bookingService.addBooking(bookingDto, booker.getId()));
        assertEquals(HttpStatus.BAD_REQUEST, exception.getStatus());
        assertEquals(exception.getMessage(), "400 BAD_REQUEST \"Вещь недоступна\"");
    ...
    void approveTest() {
        ...
        assertEquals(HttpStatus.BAD_REQUEST, exception.getStatus());
        assertEquals(exception.getMessage(), "400 BAD_REQUEST \"Бронирование уже подтверждено или отклонено\"");
    ...
    void findAllBookingsTest() {
        ...
        assertEquals(HttpStatus.BAD_REQUEST, exception.getStatus());
        assertEquals(exception.getMessage(), "400 BAD_REQUEST \"Unknown state: UNSUPPORTED_STATUS\"");
        // ALL
